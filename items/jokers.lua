...
local loyalty_tarot_joker_atlas = SMODS.Atlas{
    key = 'tarot_joker',
...
function Card:draw(layer)
    ...
    if self.sprite_facing == 'front' and self.ability.reversed then
        self.children.center:draw_shader('loyalty_reversed', nil, nil, nil, self.children.center)
    end
end
...
function get_tarot_joker()
    ...
    local _starting_pool, _pool_key = tarot_joker_pool, 'Joker'..'loyalty_tarot_joker'
...
local fool_joker = false and SMODS.Joker{
    key = 'fool_joker',
    rarity = 3,
    unlocked = false,
    discovered = false,
...
function draw_card(from, to, percent, dir, sort, card, delay, mute, stay_flipped, vol, discarded_only)
    if from == G.play and to == G.discard and #SMODS.find_card('j_loyalty_fool_joker') > 0 then
        to = G.deck
    end
    draw_card_ref(from, to, percent, dir, sort, card, delay, mute, stay_flipped, vol, discarded_only)
end

local empress_joker = SMODS.Joker{
    key = 'empress_joker',
    rarity = 3,
    unlocked = false,
    discovered = false,
...
local hermit_joker = false and SMODS.Joker{
    key = 'hermit_joker',
    rarity = 3,
    unlocked = false,
    discovered = false,
...
local strength_joker = SMODS.Joker{
    key = 'strength_joker',
    rarity = 3,
    unlocked = false,
    discovered = false,
...
local judgement_joker = SMODS.Joker{
    key = 'judgement_joker',
    rarity = 3,
    unlocked = false,
    discovered = false,
